...
* Project Abstract
** Knowledge Gap
Mutations in HIV-1 protease are a primary driver of resistance to clinically important inhibitors such as Ritonavir and Indinavir. While structure-based modeling approaches (e.g., docking and energy scoring) can estimate the impact of mutations on binding affinity, they do not effectively leverage known examples of resistant and non-resistant variants to guide design. A key knowledge gap is whether learning-based methods can improve prediction and recovery of binding affinity in mutated proteins by integrating structural and data-driven signals.
** Hypothesis
Combining structure-based modeling with contrastive learning will enable accurate identification of mutations that disrupt inhibitor binding and allow the design of compensatory mutations that restore or improve binding affinity in resistant HIV-1 protease variants.
** Data and Methods
This project integrates reinforcement learning, contrastive representation learning, and Rosetta-based structural modeling to study mutation-driven changes in protein–ligand binding for HIV-1 protease complexes with inhibitors such as Ritonavir and Indinavir.

First, structural data for HIV-1 protease bound to known inhibitors will be obtained from the Protein Data Bank. A curated subset of wild-type and experimentally characterized resistance mutations will be generated by introducing point mutations into the protein structure. These structures will serve as the initial dataset for modeling binding disruption and recovery.

Second, Rosetta-based protein modeling will be used to evaluate each protein variant. Rosetta docking and scoring functions will estimate binding affinity between protease variants and inhibitors, while additional energy terms (e.g., interface energy, packing quality, and structural stability) will be used to assess fold integrity. These scores define the physical constraints of the design space and provide the primary evaluation signal for structural quality.

Third, a reinforcement learning (RL) framework will be implemented where each state represents a partially or fully mutated protein structure, and each action corresponds to a discrete structural modification (e.g., amino acid mutation, side-chain repacking, or fragment replacement). The RL policy is guided by Rosetta-derived scores, and the reward function is defined as a weighted combination of binding affinity improvement and structural stability.

Finally, a contrastive learning model will be trained on Rosetta-evaluated structures to learn a latent embedding space that distinguishes high-affinity (wild-type-like) complexes from low-affinity (resistant mutant-like) complexes. This learned representation is incorporated into the RL reward function to bias exploration toward structurally and energetically favorable regions of sequence–structure space.

I want to create a closed loop framework to combine structural and experimental knowledge. 
